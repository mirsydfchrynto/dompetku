...
class QrisParser {
  ...
  // ── HELPER: Ekstrak Nama Penerima / Tujuan Transaksi Keluar ──
  static String _extractRecipient(String text, String fullText) {
    // 1. Tarik tunai / penarikan
    if (fullText.contains('tarik tunai') ||
        fullText.contains('menarik uang') ||
        fullText.contains('menarik saldo') ||
        fullText.contains('tarik saldo') ||
        fullText.contains('penarikan dana') ||
        fullText.contains('penarikan saldo') ||
        fullText.contains('cash out')) {
      final atMatch = RegExp(
        r'(?:di\s+|melalui\s+|via\s+)([A-Za-z0-9\s]+?)(?=\s+(?:sebesar|berhasil|pada)|[.,;\n]|$)',
        caseSensitive: false,
      ).firstMatch(text);
      if (atMatch != null) {
        final loc = atMatch.group(1)?.trim() ?? '';
        if (loc.isNotEmpty &&
            loc.length < 25 &&
            !loc.toLowerCase().contains('sebesar')) {
          return 'Tarik Tunai ($loc)';
        }
      }
      return 'Tarik Tunai';
    }

    // 2. Pembelian pulsa / paket / token / tagihan
    if (fullText.contains('tagihan')) {
      final billMatch = RegExp(
        r'tagihan\s+([A-Za-z0-9\s]+?)(?=\s+(?:sebesar|berhasil|rp)|[.,;\n]|$)',
        caseSensitive: false,
      ).firstMatch(text);
      if (billMatch != null) {
        final bill = billMatch.group(1)?.trim() ?? '';
        if (bill.isNotEmpty && bill.length < 25) {
          return 'Tagihan ${bill[0].toUpperCase()}${bill.substring(1)}';
        }
      }
      return 'Pembayaran Tagihan';
    }

    if (fullText.contains('pulsa')) return 'Pembelian Pulsa';
    if (fullText.contains('token')) return 'Token Listrik';
    if (fullText.contains('paket')) return 'Paket Data';

    // 3. Pembayaran di merchant / toko
    final atMerchantMatch = RegExp(
      r'(?:pembayaran\s+(?:di|ke)|membayar\s+(?:di|ke)|bayar\s+(?:di|ke)|belanja\s+di)\s+([A-Za-z0-9\s\.\&]+?)(?=\s+(?:sebesar|berhasil|pada|menggunakan|lewat)|[.,;\n]|$)',
      caseSensitive: false,
    ).firstMatch(text);
    if (atMerchantMatch != null) {
      final merchant = atMerchantMatch.group(1)?.trim() ?? '';
      if (merchant.isNotEmpty &&
          merchant.length < 25 &&
          !merchant.toLowerCase().contains('sebesar')) {
        return merchant;
      }
    }

    // 4. Transfer ke penerima
    final transferMatch = RegExp(
      r'(?:transfer\s+(?:uang|saldo|dana)?\s*ke|kirim\s+(?:uang|saldo|dana)?\s*ke|mengirim\s+(?:uang|saldo|dana)?\s*ke|ke\s+rekening|ke)\s+([A-Za-z0-9\s\.\&]+?)(?=\s+(?:sebesar|berhasil|pada|rek|rekening)|[.,;\n]|$)',
      caseSensitive: false,
    ).firstMatch(text);
    if (transferMatch != null) {
      var recipient = transferMatch.group(1)?.trim() ?? '';
      recipient = recipient.replaceAll(RegExp(r'[\.\,\:\;\-]+$'), '').trim();
      final lower = recipient.toLowerCase();
      if (lower.isNotEmpty &&
          lower != 'rekening' &&
          lower != 'sebesar' &&
          !RegExp(r'^\d+$').hasMatch(recipient)) {
        if (lower.startsWith('rekening ')) {
          recipient = 'Rekening ${recipient.substring(9).trim()}';
        }
        return recipient.length > 22 ? '${recipient.substring(0, 19)}...' : recipient;
      } else if (lower == 'rekening') {
        final bankMatch = RegExp(r'rekening\s+([A-Za-z0-9]+)', caseSensitive: false).firstMatch(text);
        if (bankMatch != null) {
          return 'Rekening ${bankMatch.group(1)}';
        }
      }
    }
